...
export const getDiscover = async (req, res) => {
  try {
    ...
    const data = await withCache(cacheKey, 60, () =>
      getDiscoverData(query, authProfile?.id || null),
    );
    res.json(data);
  } catch (error) {
...
export const getRecommendations = async (req, res) => {
  try {
    ...
    const data = await withCache(cacheKey, 60, () =>
      getRecommendationsData(authProfile?.id || null, query),
    );
    res.json(data);
  } catch (error) {
...
export const getTrending = async (req, res) => {
  try {
    ...
    const data = await withCache(cacheKey, 120, () =>
      getTrendingData(query, authProfile?.id || null),
    );
    res.json(data);
  } catch (error) {
...
export const getAnalytics = async (req, res) => {
  try {
    ...
    const data = await withCache(cacheKey, 300, () =>
      getAnalyticsData(authProfile.id, query),
    );
    res.json(data);
  } catch (error) {
